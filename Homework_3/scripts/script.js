...
const secondStrMinMax = () => {
    let str = '4 57 4 6 1 32 4 65 4 3 5 7 89 7 10 1 36 8 98';
    let maxNum = -Infinity;
    let minNum = Infinity;
    let flag = '';
...
const passCheck = () => {
    ...
    let pass = 's052_Y';
    let trueStr = '_*&-#$0123456789qwertyuiopasdfghjklzxcvbnmQWERTYUIOPASDFGHJKLZXCVBNM';
    let trueUpperStr = 'QWERTYUIOPASDFGHJKLZXCVBNM';
    let passCheck = '';

    for(let i = 0; i < pass.length; i++) {
        let char = pass[i];

        for(let j = 0; j < trueStr.length; j++) {
            if (char === trueStr[j]) {
                passCheck += char;
            }
        }
    ...
    for(let i = 0; i < pass.length; i++) {
        ...
        for(let j = 0; j < trueUpperStr.length; j++) {
            if (char === trueUpperStr[j]) {
    ...
    if (pass.length >= 6 && passCheck === pass && checkUpperLetter === true) {
